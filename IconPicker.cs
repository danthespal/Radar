namespace OriathHub.Plugins.Radar
{
    ...
    public class IconPicker
    {
        ...
        public void ShowSettingWidget()
        {
            ...
            if (this.showPopUp)
            {
                var popupSize = new Vector2(400f);
                var winCenter = ImGui.GetWindowPos() + ImGui.GetWindowSize() * 0.5f;
                ImGui.SetNextWindowPos(winCenter - popupSize / 2f, ImGuiCond.Appearing);
        ...
        {
            var filename = Path.GetFileName(this.FilePathName);
            var resolved = Path.Join(baseDir, filename);
            if (File.Exists(resolved))
            {
                this.FilePathName = resolved;
